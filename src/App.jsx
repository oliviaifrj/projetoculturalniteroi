...
import { 
  ...
  Menu, X, Info, ChevronRight, Barcode, QrCode, Plus, Trash2, Edit, Link as LinkIcon, Users, FileText
} from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20">
            <div className="flex items-center cursor-pointer" onClick={() => navegarPara('home')}>
              ...
              <span className="ml-3 text-xl font-bold text-[#6B4226]">Portal Cultural</span>
            </div>
...
const ProcessoCheckout = ({ evento, usuario, usuarioFirebase, db, aoCompletar, aoCancelar }) => {
  ...
  // Lógica de preços baseada na configuração do Admin
  ...
  const processarPagamento = async () => {
    ...
    try {
      // Validação 1: Verificar se está esgotado
      ...
      // Validação 2: Verificar Limite de CPF se ativado
      ...
      // Salvar Ingresso na Coleção Central
      ...
      // Incrementar o contador de ingressos do evento
      ...
      console.error(e);
      setErro("Erro ao processar ingresso. Tente novamente.");
    } finally {
        ...
        {etapa === 2 && (
          <div className="space-y-6">
            ...
            <div className="pt-6 flex justify-between items-center">
              ...
              <button onClick={processarPagamento} disabled={processando} className="px-6 py-3 bg-[#A0522D] text-white font-bold rounded-xl hover:bg-[#8B4513] transition disabled:opacity-50">{processando ? "Processando..." : (precoAtual > 0 ? "Confirmar Pagamento Simulado" : "Emitir Ingresso Gratuito")}</button>
            </div>
          </div>
        )}
  ...
  return (
    <div className="max-w-md mx-auto mt-10 bg-white p-8 rounded-2xl shadow-md border border-[#E8DCC4]">
      ...
      <form onSubmit={submeterFormulario} className="space-y-4">
        {!ehLogin && (
          <>
            <input type="text" placeholder="Nome Completo" required className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8B5A2B]" value={dadosFormulario.fullName} onChange={e => setDadosFormulario({...dadosFormulario, fullName: e.target.value})} />
            <input type="text" placeholder="CPF" required className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8B5A2B]" value={dadosFormulario.cpf} onChange={e => setDadosFormulario({...dadosFormulario, cpf: e.target.value})} />
          </>
        )}
        <input type="email" placeholder="Email" required className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8B5A2B]" value={dadosFormulario.email} onChange={e => setDadosFormulario({...dadosFormulario, email: e.target.value})} />
        <input type="password" placeholder="Senha (mín. 6 caracteres)" required minLength="6" className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8B5A2B]" value={dadosFormulario.password} onChange={e => setDadosFormulario({...dadosFormulario, password: e.target.value})} />
    ...
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#E8DCC4] flex items-center space-x-6">
      ...
      <div>
        ...
        <div className="mt-2 space-x-3">
          ...
          {usuarioApp.isAdmin && <span className="text-xs bg-teal-100 text-teal-800 px-3 py-1 rounded-full font-bold">Acesso Administrativo</span>}
        </div>
      </div>
    </div>
    <div>
          ...
          {meusIngressos.map(ingresso => (
            <div key={ingresso.id} className="bg-white rounded-xl shadow-sm border border-[#E8DCC4] overflow-hidden flex flex-col md:flex-row">
              <div className="p-6 flex-1 flex flex-col justify-center border-b md:border-b-0 md:border-r border-dashed border-[#D2B48C]">
                ...
                <p className="text-sm text-[#6B4226]"><User className="w-4 h-4 inline mr-1" /> Titular: {ingresso.ownerName} ({ingresso.ownerCpf})</p>
              </div>
...
const PainelAdministrador = ({ eventos, db }) => {
  ...
  const [dadosFormulario, setDadosFormulario] = useState({ 
    ...
    price: 0, capacity: 100, category: 'Música', isFree: false, observations: '',
  ...
  // Carrega ingressos quando o admin seleciona um evento no relatório
  useEffect(() => {
    ...
    getDocs(q).then(snap => {
      ...
      console.error(err); setCarregandoRelatorio(false);
    });
  }, [eventoSelecionadoId, db]);

  const limparFormulario = () => {
    setDadosFormulario({ title: '', description: '', date: '', time: '', location: '', imageUrl: '', price: 0, capacity: 100, category: 'Música', isFree: false, observations: '', author: '', externalLink: '', priorityType: 'meia', limitOnePerCpf: false, soldTickets: 0 });
  ...
  const deletarEvento = async (id) => {
    if (confirm("Certeza que deseja excluir este evento do portal?")) await deleteDoc(doc(db, 'events', id));
  };
          ...
          {!editando ? (
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-md border border-[#E8DCC4]">
              ...
              <form onSubmit={salvarEvento} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-1"><label className="text-sm font-medium text-[#6B4226]">Autor / Produtor (Realização)</label><input type="text" placeholder="Ex: Cia de Teatro XYZ" className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg outline-none focus:border-[#8B5A2B]" value={dadosFormulario.author} onChange={e=>setDadosFormulario({...dadosFormulario, author: e.target.value})} /></div>
                <div className="space-y-1"><label className="text-sm font-medium text-[#6B4226]">Link Externo Oficial (Opcional)</label><input type="url" placeholder="https://..." className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg outline-none focus:border-[#8B5A2B]" value={dadosFormulario.externalLink} onChange={e=>setDadosFormulario({...dadosFormulario, externalLink: e.target.value})} /></div>
                ...
                <div><label className="block text-xs text-[#A68A6B] mb-1">Data</label><input required type="date" className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg" value={dadosFormulario.date} onChange={e=>setDadosFormulario({...dadosFormulario, date: e.target.value})} /></div>
                <div><label className="block text-xs text-[#A68A6B] mb-1">Hora</label><input required type="time" className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg" value={dadosFormulario.time} onChange={e=>setDadosFormulario({...dadosFormulario, time: e.target.value})} /></div>
                ...
                <div className="col-span-2 border-t border-[#E8DCC4] pt-4 mt-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className="text-sm font-medium text-[#6B4226]">Capacidade Total</label><input required type="number" min="1" className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg" value={dadosFormulario.capacity} onChange={e=>setDadosFormulario({...dadosFormulario, capacity: e.target.value})} /></div>
                    ...
                    <div className="col-span-2"><label className="text-sm font-medium text-[#6B4226]">Configuração do "Ingresso Prioritário (Lei 10.048/00)"</label>
                      <select className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg outline-none" value={dadosFormulario.priorityType} onChange={e=>setDadosFormulario({...dadosFormulario, priorityType: e.target.value})}>
                        <option value="meia">Será cobrado o valor de Meia-Entrada</option>
                        <option value="gratuito">Será 100% Gratuito</option>
                      </select>
                    </div>
                  </div>
                </div>

                <div className="col-span-2"><label className="text-sm font-medium text-[#6B4226]">Observações (Ex: Classificação Indicativa)</label><input type="text" className="w-full px-4 py-2 border border-[#E8DCC4] rounded-lg" value={dadosFormulario.observations} onChange={e=>setDadosFormulario({...dadosFormulario, observations: e.target.value})} /></div>
      ...
      {abaAtiva === 'relatorios' && (
        <div className="bg-white p-6 md:p-8 rounded-2xl shadow-md border border-[#E8DCC4] space-y-6">
          <div>
            ...
            <select className="w-full max-w-lg px-4 py-3 border border-[#E8DCC4] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#A0522D] bg-[#FDFBF7]" value={eventoSelecionadoId} onChange={(e) => setEventoSelecionadoId(e.target.value)}>
              ...
              {carregandoRelatorio ? (
                ...
                <>
                  ...
                  {ingressosVendidos.length === 0 ? (
                    <div className="text-center py-8 bg-[#FDFBF7] rounded border border-dashed border-[#D2B48C] text-[#A68A6B]">Nenhum ingresso foi gerado para este evento ainda.</div>
                  ) : (
